...
import { Search, X, Check, RotateCcw, Plus, Info } from 'lucide-react';
...
// ─────────────────────────────────────────────────────────────────────────────
// ДЕТАЛЬНАЯ КАРТОЧКА УПРАЖНЕНИЯ (bottom-sheet поверх пикера)
// ─────────────────────────────────────────────────────────────────────────────
function ExerciseDetailSheet({ exercise, onClose, onAdd, isInWorkout }) {
  ...
  const loadEntries = Object.entries(loads)
    .filter(([k]) => MUSCLES[k])
    .sort(([,a],[,b]) => b - a);
  const isPrimary = (k) => keys.includes(k);
  ...
  return (
    ...
    >
      <div
        className="bg-dark-surface rounded-t-2xl flex flex-col overflow-hidden"
        style={{ maxHeight: '88%' }}
      ...
      >
        ...
        {/* Шапка с цветным акцентом */}
        <div className="px-4 pt-2 pb-3 flex-shrink-0"
          style={{ borderBottom: `1px solid ${primaryColor}25` }}>
          <div className="flex items-start gap-3">
            {/* Цветная иконка */}
            <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 text-lg font-bold"
              style={{ backgroundColor: `${primaryColor}20`, border: `1.5px solid ${primaryColor}40`, color: primaryColor }}>
              {exercise.name_ru?.[0] ?? '?'}
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="font-bold text-base leading-tight">{exercise.name_ru}</h3>
              <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                <span className="text-xs font-medium" style={{ color: primaryColor }}>
                  {MUSCLES[primaryKey]?.ru ?? exercise.primary_muscle}
                </span>
                {exercise.equipment && (
                ...
                {exercise.difficulty && (
                  <span className={`text-xs px-1.5 py-0.5 rounded-md border
                    ...
                    {exercise.difficulty === 'beginner' ? 'начальный'
                      : exercise.difficulty === 'advanced' ? 'продвинутый' : 'средний'}
                  </span>
                )}
                {exercise.is_compound && (
                  <span className="text-xs px-1.5 py-0.5 rounded-md border bg-blue-500/10 text-blue-400 border-blue-500/20">
                    база
                  </span>
                )}
              </div>
            ...
            <button onClick={onClose}
              className="p-1.5 text-dark-muted hover:text-dark-text rounded-lg hover:bg-dark-elevated transition-colors flex-shrink-0 mt-0.5">
              <X className="w-4 h-4"/>
            </button>
          </div>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-4 py-3 space-y-4">
          ...
          {loadEntries.length > 0 && (
            <div>
              <p className="text-[10px] uppercase tracking-widest text-dark-muted mb-2 font-medium">
                Нагрузка по мышцам
              </p>
              {/* Полный стек-бар */}
                ...
                {loadEntries.map(([muscleKey, pct]) => {
                  ...
                  return (
                    <div key={muscleKey}
                      style={{
                        width: `${pct}%`,
                        backgroundColor: isP ? c : `${SECONDARY_COLOR}bb`,
                        transition: 'width 0.4s ease',
                      }}
                      title={`${MUSCLES[muscleKey].ru}: ${pct}%`}
                    />
                  );
                })}
              </div>
              ...
              <div className="space-y-1.5">
                {loadEntries.map(([muscleKey, pct]) => {
                  const c = MUSCLES[muscleKey].color;
                  const isP = isPrimary(muscleKey);
                  const barColor = isP ? c : SECONDARY_COLOR;
                  return (
                    <div key={muscleKey}>
                      <div className="flex items-center justify-between mb-0.5">
                        <div className="flex items-center gap-1.5">
                          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: barColor }}/>
                          <span className="text-xs" style={{ color: isP ? `${c}ee` : `${SECONDARY_COLOR}99` }}>
                            {MUSCLES[muscleKey].ru}
                          </span>
                          {!isP && (
                            <span className="text-[9px] text-dark-muted uppercase tracking-wide">антагонист</span>
                          )}
                        </div>
          ...
          {/* Описание */}
          {exercise.description_ru && (
            <div>
              <p className="text-[10px] uppercase tracking-widest text-dark-muted mb-1.5 font-medium">Описание</p>
              <p className="text-sm text-dark-text leading-relaxed">{exercise.description_ru}</p>
            </div>
          )}

          {/* Техника */}
          {exercise.instructions_ru?.length > 0 && (
            <div>
              <p className="text-[10px] uppercase tracking-widest text-dark-muted mb-2 font-medium">Техника</p>
              <div className="space-y-1.5">
                {exercise.instructions_ru.map((step, i) => (
                  <div key={i} className="flex gap-2.5 items-start">
                    ...
                    <p className="text-sm text-dark-text leading-snug flex-1">{step}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Советы */}
          {exercise.tips_ru?.length > 0 && (
            <div>
              <p className="text-[10px] uppercase tracking-widest text-dark-muted mb-2 font-medium">Советы</p>
              <div className="space-y-1.5">
                {exercise.tips_ru.map((tip, i) => (
                  <div key={i} className="flex gap-2 items-start px-3 py-2 rounded-lg"
                    style={{ backgroundColor: `${primaryColor}10`, border: `1px solid ${primaryColor}20` }}>
                    <span style={{ color: primaryColor }} className="flex-shrink-0 mt-0.5">💡</span>
                    <p className="text-xs text-dark-text leading-snug">{tip}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Кнопка добавить */}
        <div className="px-4 pb-6 pt-2 flex-shrink-0 border-t border-dark-border">
          {isInWorkout ? (
            <div className="flex items-center justify-center gap-2 py-3 text-primary-400">
              <Check className="w-4 h-4"/>
              <span className="text-sm font-medium">Уже добавлено в тренировку</span>
            </div>
          ) : (
            <button
...
export default function ExercisePicker({
  ...
  function handleOpenDetail(ex) {
    setDetailExercise(ex);
  }

  // Добавить упражнение (не закрывает модалку)
  ...
  return (
    <div className="fixed inset-0 bg-black/85 z-50 flex items-end sm:items-center justify-center">
      <div
        className="bg-dark-surface w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl flex flex-col overflow-hidden relative"
        style={{ maxHeight: 'min(94vh, 740px)' }}
      >
        {/* Детальная карточка — слой поверх */}
        {detailExercise && (
          <ExerciseDetailSheet
            exercise={detailExercise}
            onClose={() => setDetailExercise(null)}
            onAdd={() => handleAdd(detailExercise)}
            isInWorkout={selectedIds.has(detailExercise.id)}
          />
        )}
        {/* Шапка */}
                ...
                return (
                  <div key={exercise.id}
                    ...
                    style={isHighlighted && !inWorkout && !inMulti
                      ? { borderColor: `${primaryColor}35`, backgroundColor: `${primaryColor}06` }
                      : {}}>

                    {/* ── ЛЕВАЯ ЧАСТЬ: нажать → открыть детали ── */}
                    <button
                      onClick={() => handleOpenDetail(exercise)}
                      className="flex-1 text-left flex flex-col gap-1.5 px-3 py-2.5 min-w-0 hover:opacity-90 transition-opacity">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        {/* Иконка «подробнее» */}
                        <Info className="w-3.5 h-3.5 flex-shrink-0 opacity-30"/>
                      </div>
                            ...
                            {loadEntries.map(([muscleKey, pct]) => {
                              ...
                              return (
                                <div key={muscleKey} className="flex items-center gap-1">
                                  ...
                                  <span className="text-[10px]" style={{ color: isP ? `${c}dd` : `${SECONDARY_COLOR}88` }}>
                                    {MUSCLES[muscleKey].ru}
                                  </span>
                                  <span className="text-[10px] font-bold" style={{ color: isP ? c : `${SECONDARY_COLOR}aa` }}>
                                    {pct}%
                                  </span>
                                </div>
                              );
                    ...
                    {/* ── ПРАВАЯ ЧАСТЬ: нажать → добавить (не закрывает) ── */}
                    {!multiSelect && (
                      <button
                        onClick={() => handleAdd(exercise)}
                        className="flex-shrink-0 flex items-center justify-center w-12 transition-all group"
                        style={{
                          borderLeft: `1px solid ${inWorkout ? primaryColor + '30' : '#1e1e38'}`,
                        }}
                        title={inWorkout ? 'Добавить ещё сет' : 'Добавить упражнение'}>
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center transition-all
                          ${inWorkout
                            ? 'bg-primary-600/20 group-hover:bg-primary-600/40'
                            : 'bg-dark-elevated group-hover:scale-110 group-hover:shadow-lg'}`}
                          style={!inWorkout ? {
                            boxShadow: 'none',
                          } : {}}
                          onMouseEnter={e => {
                            if (!inWorkout) {
                              ...
                              e.currentTarget.style.boxShadow = `0 0 8px ${primaryColor}40`;
                            }
                          }}
                          onMouseLeave={e => {
                            if (!inWorkout) {
                          ...
                          {inWorkout
                            ...
                            : <Plus className="w-4 h-4 text-dark-muted group-hover:text-white transition-colors"/>}
                        </div>
                      </button>
                    )}
                  </div>
